...
public class ModArmorItem extends ArmorItem {
    private static final Map<ArmorMaterial, MobEffectInstance> MATERIAL_TO_EFFECT_MAP = (new ImmutableMap.Builder<ArmorMaterial, MobEffectInstance>())
            .put(ModArmorMaterials.AMETHYST, new MobEffectInstance(MobEffects.DAMAGE_BOOST, 200, 1, false, false, true)).build();
    ...
    private void evaluateArmorEffects(Player player) {
        for(Map.Entry<ArmorMaterial, MobEffectInstance> entry : MATERIAL_TO_EFFECT_MAP.entrySet()) {
            ...
            if (hasCorrectArmorOn(mapArmorMaterial, player)) {
                addStatusEffectMaterial(player, mapArmorMaterial, mapStatusEffect);
            }
        }
    }
    ...
    private void addStatusEffectMaterial(Player player, ArmorMaterial mapArmorMaterial, MobEffectInstance mapStatusEffect) {
